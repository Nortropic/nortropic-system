...
## H-035 R32 request-bound registry consumer refreeze
...
registry from the same request. R32 therefore authorizes exactly two product files:
...
core import and named legacy request-bound islands is AST-identical to that base. R32 closes those
...
Standard preproduct execution is `--r32-registry-migration`; product acceptance is
`--r32-registry-migration BASE_SHA CANDIDATE_SHA`. The sole preproduct RED is
`A_R32_REQUEST_BOUND_REGISTRY_AND_NUL_PATH_OPERATIONAL` with reason
`LEGACY_CORE_CLI_LACK_V3_NUL_AND_REQUEST_BOUND_OWNER_SURFACE`. Product identity is a clean direct
single child of the published R32 merge, changes exactly the two authorized product files with modes
...
publication, R32 proceeds through an independent two-file product review and guarded publication,
