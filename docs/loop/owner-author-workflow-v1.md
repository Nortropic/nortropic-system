...
selected-secret absence are not evidence. Argv is tokenized, not matched by a substring regular
expression. Every descriptor argument is a canonical unsigned decimal integer whose parsed value
is at least 3; values `3`, `9`, `10`, `29` and larger legitimate descriptors are accepted while
`0`, `1`, `2`, signs, leading zeroes, fractions and non-decimal text reject.
...
or reserved environment names likewise reject. BASE, WITH_STAGING and a later top-level launch have
...
reference runner owns one session/process group, keeps the exited leader unreaped while classifying
exact members, closes sockets before roots, signals only the bound group before reaping and proves
forced postspawn hang/error remain non-PASS with two stable absence samples, no zombie and no root.
