...
lines, and unchanged frozen task/gate bytes. H-017 is preserved historical evidence only and never
...
The fixed cumulative profile keeps every ancestor live/trust/runtime/Git/workspace/staging denial,
never grows a Git or sink exception, and narrows nested writes to the nofollow-resolved child
workspace. An optional staging exception permits file data at only the already-open exact sink;
...
helper under a closed environment. The post-profile syscall proof is mandatory: before target env
or exec, that helper must demonstrate real permitted scratch/exact-sink writes and denied
create/write/rename/unlink/hardlink/mkdir operations at every cumulative protected boundary. The
supervisor's authenticated proof receipt is supporting evidence only; the gate observes the actual
effects. An inner direct sandbox-exec control still returns 71 with
...
Success preserves target stdout byte-for-byte. Success, launch failure, nonzero, timeout, SIGTERM,
disconnect and supervisor death each revoke handoff and clean the known leader/process group plus
