...
## H-035 R28 three-state owner-production registry migration
...
domain with the minimum OS-exclusive cleanup authority. H-035 R28 does not create H-039, install a
...
duplicate fixed R14 product-scope temp root and missing causal valid-v3 positives. R28 is a fresh
direct child of `c161a1c557b58516f3881ac9f49f4e50216c8bc0` and executes the inherited R14
...
remain unchanged. The R28 BUILDER may change exactly `controller/authority/core.py`; the contract
...
Standard preproduct execution is exactly `--r28-registry-migration`; product acceptance is
`--r28-registry-migration BASE_SHA CANDIDATE_SHA`. Both run the complete historical R14 matrix and
the R28 incremental matrix. The only authorized preproduct RED is
`A_R28_EXACT_THREE_STATE_REGISTRY_OPERATIONAL` with reason
...
Serial order is H035 R28 freeze → two independent gate reviews → guarded contract publication →
