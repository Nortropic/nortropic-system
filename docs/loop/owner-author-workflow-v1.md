...
helper/config/profile/live-root override negatives. It does not overclaim a universal same-UID
pre-sandbox race experiment: reachability and order of same-opened live-root/config/helper identity
and final-use rehash are mandatory immutable-object review findings.
...
replaces instead of preserving these roles rejects. At each proof barrier `ps eww` must full-match
the entire supervisor/helper argv and exact three-entry closed environment; substring hits or
selected-secret absence are not evidence.
...
Every required header key and exact operation is mandatory. Booleans and equal-valued floats are
not integers; non-finite numbers, wrong scalar/container types, missing keys and duplicate or
reserved environment names reject under fresh nonces. Every parent and sibling capability is
pairwise distinct—not merely different from its parent—while session and level remain causal. A
confined descendant with every `NORTROPIC_H036_*` key removed must enter public bootstrap, meet the
real nested Seatbelt EPERM, return class 3, start no target and leave no runtime root; it may never
catch that error and use raw `Popen`.
...
missing/nonregular/symlink/hardlink sink and staging overlap with live/trust/Git/runtime/workspace all
reject before launch. `/var` and `/private` aliases bind the same dev/ino; overlaps and escape
components reject. Duplicate, oversized, partial or trailing request frames spawn no child, and the
...
symlink rejection, real `/var`→`/private` workspace/sink readback, and alias-overlap rejection.
...
reachable stable-open/final-rehash and template-selection ordering; dynamic effects own subject
credit.
...
not claim complete absence of an escaped same-UID descendant on macOS. Instead a setsid survivor
must retain the installed policy and be unable to mutate live, trust or runtime roots; the later
H-032 protected materialization boundary closes late workspace effects.
