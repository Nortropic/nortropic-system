...
Independent review rejected initial H-034 gate candidate `45c3ffa70cf4c91a2baf6890101cd4436824c85c` on `H034-GR-01..03`. R2 proves the supported owner-toolchain CDHash observation with a disposable signed Mach-O and uses the exact `codesign -dvvv --` invocation. Deployment-minimum exclusion covers direct flags, linker platform-version forms and version-bearing macOS/Darwin target triples while retaining versionless arm64 triples and manifest-binding actual Mach-O minOS/SDK. Black-box H-033 semantic inputs are now fresh gate-generated families: candidate/spec/gate/request/probe/path/blob/marker values and receipt digests vary every run, with independently mutated valid and invalid relations through inherited descriptors. Gate randomness creates test cases only and is not runtime authority. H-033 still owns protected opening/origin. No production, registry, H-035, AUTOPILOT, push or merge change is included.

Independent review rejected R2 candidate `ba3a7d25d096e5e7ca6303632465921a8f898c7b` on `H034-R2-GR-02` because joined `-Wl,-platform_version,...` and `-Wl,-macosx_version_min,...` bypassed its selector classifier; a broader permissive C semantic mutant remained uncompleted. R3 replaces the blacklist with one exact closed compile argv: fixed C11/O2/warnings-as-errors, arm64, exact resolved sysroot, deterministic no-UUID and the complete sorted C source list only. There is no response-file, linker-pass-through, target-triple, deployment-selector or extension slot; build environment remains closed and signing has a fixed identifier independent of output basename. Metamorphic semantic controls now remove every required top-level evidence/receipt field and every nested allowlist-probe field, and test unexpected/reordered documents across multiple fresh families. A disposable permissive substring parser with real receipt SHA must fail for semantic reasons. H-033 same-FD/protected-origin division remains unchanged; no production or authority-registry edit.
...
Independent review rejected H-034 R3 because mandatory `-Wl,-no_uuid` produced a signed artifact that target dyld would not launch, while unbounded gate subprocesses could hang and leave external uninterruptible residue. R4 restores a normal loadable LC_UUID, executes a signed owner-toolchain fixture before semantic tests, and bounds every disposable/kernel child in an isolated process group with cleanup and explicit fast/slow/hang controls. Reproducibility is now empirical and honest: identical unsigned compilations to a canonical basename plus stable semantic/Mach-O/dependency/manifest identity are required; independently ad-hoc-signed byte identity is not assumed, while the committed final signed digest and CDHash remain exact authority. No production, H-035, registry, AUTOPILOT, push or merge change is included.
