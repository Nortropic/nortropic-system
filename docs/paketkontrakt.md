...
|---|---|---|
| `PK-GAP-1` | **STÄNGD 2026-08-27 (HÖGRISK, ägarinstruktion).** Paket↔kapacitet var inte bundet till paket-id: manifestet pekade på katalogen i allmänhet och namngav ingen enskild `KAP-`-rad. Luckan sa att *"att kräva bindningen vore att kräva något referensen inte uppfyller"* — **så referensen fick bära den först.** `packs/lokal-se/manifest.md` bär nu en kapacitetskomposition med sex rader och tre roller, och kontraktet KRÄVER den. **`ÄRVER` är en egen roll och inte en tystnad:** en kapacitet som varken ägs eller skärps måste ändå stå med, annars går "gäller oförändrad" inte att skilja från "glömdes bort". Manifestet är §A7-yta; ändringen är gjord på uttrycklig ägarinstruktion och HÖGRISK-märkt | Masterplanens D2 förutsatte bindningen. **`PK-GAP-4` STÄNGD 2026-08-27:** kompositionen HÄRLEDS nu ur något som inte är manifestet — paketets egna fixturer. En fixtur med `paket: ['lokal-se']` deklarerar i `kapaciteter` vad bygget faktiskt aktiverar, och **kompositionen måste TÄCKA varje sådan kapacitet**. En lista som bara granskas för det den innehåller kan alltid krympas; den här kan inte det. Riktningen är medvetet ensidig — en komposition FÅR bära en rad ingen fixtur ännu övar, men aldrig sakna en som övas, och oövade rader RAPPORTERAS i stället för att fällas. **Ankarkravet gäller:** ett paket utan fixturer ger FAIL, inte grönt — utan underlag går härledningen inte att göra, och det ska sägas |
...
| `PK-GAP-3` | **Masterplanens §6 räknar upp SEX paketdelar** — manifest, research module, strategy module, **eval module**, **gate lenses**, **agent fragments**. **DELVIS ÅTGÄRDAT 2026-08-27: gate lenses är byggd** (`packs/lokal-se/gate-lenses.md`). Kontraktet kräver fortfarande bara de tre första plus katalograd, statusrad och pinne; eval module och agent fragments saknas | Linserna FANNS redan — men i `workflows/nortropic-launch.js`, inte i paketet. **Ett andra paket kunde därmed inte tillföra en lins utan att någon redigerade grindworkflowet**, vilket är `PK-GAP-1`:s arkitekturfynd i konkret form: masterplanens D2 säger att *"packs are named compositions of capabilities"*, och en komposition som kräver att värden ändras är ingen komposition. Paketet deklarerar nu sina linser och `scripts/check-paketlinser.mjs` fäller i BÅDA riktningar om deklarationen och den hårdkodade tabellen glider isär. **Kvarstår som `GL-GAP-1`:** workflowet LÄSER inte paketet — tabellen är en spegel, och en spegel är ingen koppling. Att koppla den kräver en ändring i `nortropic-launch.js` vars kategorimängd gränsar till §A3 |
