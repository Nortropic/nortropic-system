...
license: Apache 2.0
allowed-tools:
  - Bash(node */vendored-skills/impeccable/scripts/*)
  - Bash(node .claude/skills/impeccable/scripts/*)
---

Designs and iterates production-grade frontend interfaces. Real working code, committed design choices, exceptional craft.

> **LOCAL_NORTROPIC_FORK** of `pbakaus/impeccable@ddc37242` (Apache 2.0) with a closed Nortropic patchset — no self-update, no automatic dependency installation, no automatic CSP broadening. Full provenance and patchset: `VENDORED.md`. Upstream changes reach this fork only via Radar → kandidat → diff → granskning → promotion.

## Setup

You MUST do these steps before proceeding:

1. Run `node <skill-base-dir>/scripts/context.mjs` once per session, where `<skill-base-dir>` is the directory this SKILL.md was loaded from (repo-native authority: `…/vendored-skills/impeccable`; a legacy `.claude/skills/impeccable` projection is only valid if it actually exists AND is byte-identical to the repo copy — the repo wins, per doctor #9). Keep cwd/workdir at the user's project, not the skill directory. If the request names or implies a file, route, or app inside a monorepo, infer the concrete path and append `--target <path>` to the same command. If you've already seen its output in this conversation, do not re-run it. The script either prints the project's PRODUCT.md (and DESIGN.md when present) as a markdown block, or tells you it's missing. Follow whatever it prints. **If it reports `NO_PRODUCT_MD`:** divert into `reference/init.md` first when the user invoked `init`, `teach`, `craft`, or `shape`, or when their wording clearly maps to one of those from-scratch build flows (for example: "build/create/make a landing page", "design a new app", or "shape a feature"). Captured product context is the point of those flows. For any other command, a scoped evaluate / refine / enhance / fix / iterate request against existing code, do **not** divert into init. The existing code is the context: proceed with the requested command, infer the register from the surface in focus (step 4), and offer `/impeccable init` once as a suggestion the user can take later. A missing PRODUCT.md must never block a scoped request.
