...
description: Search a codebase or UI for places that don't animate but should, and reject everything that shouldn't. Read-only; it proposes motion with exact values, it does not implement it. Use when the user asks "what could be animated here?" or wants to "make this feel more alive". Reviewing or fixing existing animations is not this skill's job — that belongs to Nortropic's ordinary design-review lane.
---

# Finding Animation Opportunities

A search skill. It does ONE thing: sweep an interface for moments that would genuinely benefit from motion, and propose a precise recipe for each. It does not review existing animations, audit and plan fixes for them, or write the implementation itself — in Nortropic those belong to the ordinary route: `stack-builder` implements accepted suggestions (within the brief's Motion-nivå) and `design-reviewer`'s canon reviews the result.
...
1. **Never modify source code.** This skill reports; it does not implement. If asked to build a suggestion, hand it off — each recipe row is the handoff artifact: `stack-builder` implements it (within the brief's Motion-nivå), or let the user take the recipe to any agent.
...
One short paragraph: how much motion this interface actually needs, whether it's already close to right, and which single suggestion has the highest leverage. Close by pointing at the handoff: each row is a self-contained recipe for `stack-builder` (or any implementing agent) to build within the brief's Motion-nivå; the result is reviewed by `design-reviewer`'s canon.
