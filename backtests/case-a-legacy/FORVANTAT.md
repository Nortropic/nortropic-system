...
| `AL-GAP-3` | **STÄNGD 2026-08-27.** Luckan sa att *"formen prövas mot minnet av en version, inte mot versionen"* — och **minnet var fel i BÅDA riktningarna.** v3.0.0 är nu frusen byte-oförändrad i `config/frusna-kontrakt/`, verifierad mot den historiska git-bloben `6e7a94e5`, och legacyfixturen prövas mot DEN i stället för mot v3.1.0 plus ett handskrivet undantag. **(i)** Vaktens undantag lät `§1` heta `NAP` med motiveringen *"v3.0.0 där §1 hette så"* — v3.0.0:s §1 hette `Organisation & typade kontaktvägar`, identiskt med v3.1.0; `NAP` stod i §1:s BESKRIVNING, aldrig som namn. Dispensen vilade på en falsk premiss, och ett minne kan inte falsifiera sig självt. **(ii)** Fixturen bar `## 5. Geografisk räckvidd & språk` — **v3.1.0:s** namn. v3.0.0 säger `Geografi & språk`. Filen som finns för att bevisa det GAMLA kontraktet bar det NYAS rubrik, och ingen kontroll kunde se det. Båda rubrikerna är rättade till v3.0.0:s faktiska namn. **Kvarstår (inte en halv stängning, en annan fråga):** att FORMEN nu prövas mot rätt version säger ingenting om BETEENDET. `AL-11` och `AL-12` kräver två verkliga byggen och är fortfarande `EJ KÖRDA`. Ankarkravet är kod: skiljer sig v3.0.0 och v3.1.0 inte längre i något sektionsnamn blir kontrollen FAIL, eftersom kompatibilitetsvägen då inte prövar någon versionsskillnad | `STÄNGD` |
