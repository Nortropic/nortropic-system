...
VIM_PIN="c5148ad96932d57af7a6415ad43a7189f2b689aa839fa1fe04265dbc019f8cb1"
...
WQP_PIN="9d837e88e1824fe6a4dd61d87bf38c04dfca1a05ef429bcdfb2bdaa50d2c92c3"
WQL_PIN="e6dc3c8ae53d5ac2e3df42076f1bf9851a9a9187ec7cea3c9152f9305f1ef31d"
...
# ---- K7: kanonisk pinnad Lighthouse-väg (R7) — mätsticka + INP-sanningsgräns -
RLH="$ROT/scripts/run-lighthouse-gate.mjs"
[ -f "$RLH" ] || fail "K7: kanoniska Lighthouse-runnern saknas"
RLH_PIN="e60bc63d9c70582969909f9fd3ebdf9f2715bdd1fb998e895c4555105ffde9a7"
[ "$(shasum -a 256 "$RLH" | awk '{print $1}')" = "$RLH_PIN" ] || fail "K7: runnerkällan avviker från foundation-pinnen — mätstickan ändrad utan skyddad transition"
grep -q '"lighthouse": "13.4.1"' "$WQP" || fail "K7: lighthouse inte exakt-pinnad 13.4.1 i verktygskontraktet"
grep -q "REQUIRED_LH_VERSION = '13.4.1'" "$RLH" || fail "K7: förväntad Lighthouse-identitet 13.4.1 saknas i runnern"
grep -q "node_modules', 'lighthouse', 'cli', 'index.js'\|'lighthouse', 'cli', 'index.js'" "$RLH" || fail "K7: direkt pinnad CLI-väg saknas (ankarkrav)"
grep 'npx' "$RLH" | grep -vi 'aldrig\|never\|inget' | grep -q . && fail "K7: aktiv npx-/global-väg återinförd i runnern"
grep -qE 'no-sandbox|disable-setuid-sandbox|disable-web-security|ignore-certificate-errors' "$RLH" && fail "K7: webbläsarsäkerhets-bypass återinförd i Lighthouse-runnern"
grep -q "only-categories=\${CATEGORIES" "$RLH" || fail "K7: kategoribindningen saknas (ankarkrav)"
grep -q "CATEGORIES = \['performance', 'accessibility', 'best-practices', 'seo'\];" "$RLH" || fail "K7: exakta kategorimängden ändrad (sluten mängd — varken breddad eller smalnad)"
grep -q "formFactor !== 'mobile'" "$RLH" || fail "K7: mobil-ankaret saknas — desktop får aldrig passera tyst"
grep -q "lighthouseVersion !== REQUIRED_LH_VERSION" "$RLH" || fail "K7: LHR-versionsidentitetskontrollen saknas"
grep -q 'LCP_MAX_MS = 2500' "$RLH" || fail "K7: LCP-tröskelankaret saknas/ändrat"
grep -q 'CLS_MAX = 0.1' "$RLH" || fail "K7: CLS-tröskelankaret saknas/ändrat"
grep -q 'NOT_MEASURED_BY_NAVIGATION_LIGHTHOUSE' "$RLH" || fail "K7: INP-sanningsgränsen saknas i runnern"
grep -q 'INP_PASS' "$RLH" && fail "K7: INP-PASS-syntes ur navigation/TBT planterad — förbjudet"
grep -q 'run-lighthouse-gate.mjs' "$ROT/skills/nortropic-prelaunch/SKILL.md" || fail "K7: Gate 2 pekar inte på den kanoniska runnern"
grep -q 'MCP-/DevTools-Lighthouse förblir observationsinstrument, aldrig kanonisk' "$ROT/skills/nortropic-prelaunch/SKILL.md" || fail "K7: observations-spärren saknas i Gate 2 (ankarkrav)"
