...
class Supervisor:
    ...
    def destroy(self) -> None:
        ...
        if not self.preserve_root: self._remove_root()

    def _remove_root(self) -> None:
        root = self.cfg.get("root")
        if not root:
            return
        if os.path.lexists(os.path.join(root, "k", ".nortropic-h038-cleanup-unsafe")): return
        try:
            os.chmod(root, 0o700)
        except OSError:
            pass
    ...
    def _cleanup_neutral(self, state) -> bool:
        ...
        restorable = state.get("published") is True and state.get("complete") is True and set(sources) == expected and len(identities) == 4; restored = marker_created = False; marker_fd = -1; marker = ".nortropic-h038-cleanup-unsafe"
        try:
            if created and directory_identity is not None and min(attempt_fd, origin_fd, directory_fd) >= 0:
                marker_fd = os.open(marker, os.O_WRONLY | os.O_CREAT | os.O_EXCL | os.O_NOFOLLOW | getattr(os, "O_CLOEXEC", 0), 0o600, dir_fd=origin_fd); marker_created = True; marker_item = os.fstat(marker_fd)
                if not stat.S_ISREG(marker_item.st_mode) or marker_item.st_uid != os.geteuid() or stat.S_IMODE(marker_item.st_mode) != 0o600 or marker_item.st_nlink != 1: raise ValueError("unsafe cleanup marker")
                marker_owned, marker_fd = marker_fd, -1
                if not close_owned(marker_owned): raise OSError(errno.EIO, "cleanup marker close failed")
            ...
            if private_name is not None:
                ...
                if bound:
                    ...
                    if not remaining: os.rmdir(private_name, dir_fd=origin_fd); restored = True
                    elif not (restorable and seen == identities): ok = False
        except (OSError, ValueError): ok = False
        finally:
            ok = close_owned(marker_fd) and ok
            if private_name is not None and restorable and not restored: restored = self._move_identity(origin_fd, private_name, attempt_fd, state.get("public_name"), directory_identity)
            if marker_created and (private_name is None or restored) and not self.preserve_root:
                try: os.unlink(marker, dir_fd=origin_fd)
                except OSError: self.preserve_root = True
