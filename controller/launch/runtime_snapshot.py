...
class Supervisor:
    ...
    def _park_identity(self, parents, identity, origin_fd):
        for _ in range(8):
            for parent_fd, entry in ((fd, entry) for fd in parents for entry in os.listdir(fd)):
                    try: item = os.stat(entry, dir_fd=parent_fd, follow_symlinks=False)
                    except OSError: continue
                    if (item.st_dev, item.st_ino) != identity: continue
                    target = ".nortropic-h038-quarantine-" + secrets.token_hex(32); moved_ok = True
                    try: rename_exclusive(parent_fd, entry, origin_fd, target)
                    except OSError: moved_ok = False
                    try: moved = os.stat(target, dir_fd=origin_fd, follow_symlinks=False)
                    except OSError: continue
                    if (moved.st_dev, moved.st_ino) == identity: return target, moved_ok
                    if moved_ok: rename_exclusive(origin_fd, target, parent_fd, entry)
        return None, False
    def _cleanup_neutral(self, state) -> bool:
        ...
        restorable = state.get("published") is True and state.get("complete") is True and set(sources) == expected and len(identities) == 4 and ok
        try:
            if created and directory_identity is not None and min(attempt_fd, origin_fd, directory_fd) >= 0: private_name, moved_ok = self._park_identity((origin_fd, attempt_fd), directory_identity, origin_fd); found = private_name is not None; ok = moved_ok and ok
            if private_name is not None:
                held, moved = os.fstat(directory_fd), os.stat(private_name, dir_fd=origin_fd, follow_symlinks=False); bound = (held.st_dev, held.st_ino) == directory_identity == (moved.st_dev, moved.st_ino) and stat.S_ISDIR(moved.st_mode) and moved.st_uid == os.geteuid() and stat.S_IMODE(moved.st_mode) == 0o700; ok = bound and ok
                if bound:
                    for identity in identities:
                        parked, moved_ok = self._park_identity((directory_fd,), identity, origin_fd)
                        if parked is None: continue
                        item = os.stat(parked, dir_fd=origin_fd, follow_symlinks=False); ok = moved_ok and stat.S_ISREG(item.st_mode) and item.st_uid == os.geteuid() and stat.S_IMODE(item.st_mode) == 0o600 and item.st_nlink == 1 and ok; os.unlink(parked, dir_fd=origin_fd); seen.add(identity)
                    remaining = os.listdir(directory_fd); ok = all(((item := os.stat(entry, dir_fd=directory_fd, follow_symlinks=False)).st_dev, item.st_ino) not in identities for entry in remaining) and ok
                    if not remaining: os.rmdir(private_name, dir_fd=origin_fd)
                    elif restorable and ok and seen == identities:
                        public_name = state.get("public_name"); rename_exclusive(origin_fd, private_name, attempt_fd, public_name); restored = os.stat(public_name, dir_fd=attempt_fd, follow_symlinks=False); ok = (restored.st_dev, restored.st_ino) == directory_identity and ok
                    else: ok = False
        except OSError: ok = False
        finally:
