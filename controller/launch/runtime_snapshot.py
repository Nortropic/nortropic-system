...
class Supervisor:
    ...
    def _profile_params(self, current: str, current_identity=None) -> tuple[dict[str, str], str, list[str]]:
        ...
        if cfg.get("profile") == "managed":
            params.update({"ATTEMPT_ROOT": self._bound_managed(current, current_identity)["attempt"], "STAGING_ROOT": cfg["staging_root"], "RESULT_SINK": cfg["result_sink"]})
    ...
    def _neutral_attempt(self, request_nonce: str, attempt: str, state: dict):
        flags = os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0) | getattr(os, "O_CLOEXEC", 0); attempt_fd = os.open(attempt, flags)
        token = secrets.token_hex(32)
        while token == request_nonce: token = secrets.token_hex(32)
        name = ".nortropic-h038-proof-" + token
        state.update({"attempt_fd": attempt_fd, "directory_fd": -1, "directory_identity": None, "directory_created": False, "source_identities": set(), "pending_source_fd": -1})
        opened_attempt = os.fstat(attempt_fd)
        if [opened_attempt.st_dev, opened_attempt.st_ino] != self.cfg["managed_identities"]["attempt"]: raise ValueError("attempt root changed before proof setup")
        os.mkdir(name, 0o700, dir_fd=attempt_fd); state["directory_created"] = True
        created = os.stat(name, dir_fd=attempt_fd, follow_symlinks=False); state["directory_identity"] = (created.st_dev, created.st_ino); directory_fd = os.open(name, flags, dir_fd=attempt_fd)
        ...
        for source in ("write-source", "rename-source", "unlink-source", "hardlink-source"):
            fd = os.open(source, os.O_WRONLY | os.O_CREAT | os.O_EXCL | os.O_NOFOLLOW | getattr(os, "O_CLOEXEC", 0), 0o600, dir_fd=directory_fd); state["pending_source_fd"] = fd
            item = os.fstat(fd); state["source_identities"].add((item.st_dev, item.st_ino))
    ...
    def _cleanup_neutral(self, state) -> bool:
        ...
        attempt_fd, directory_fd, pending_fd = state.get("attempt_fd", -1), state.get("directory_fd", -1), state.get("pending_source_fd", -1)
        identities, directory_identity = state.get("source_identities", set()), state.get("directory_identity"); created = state.get("directory_created", False)
        ok, found, seen = not created or directory_identity is not None, not created, set()
        try:
            if pending_fd >= 0: item = os.fstat(pending_fd); identities.add((item.st_dev, item.st_ino))
            ...
            for descriptor in (pending_fd, directory_fd, attempt_fd):
                if descriptor >= 0:
                    try:
                        os.close(descriptor)
                    except OSError: ok = False
            state["attempt_fd"] = state["directory_fd"] = state["pending_source_fd"] = -1
    ...
    def _run_launch(self, conn, header, fds, ctx, envelope):
        ...
        try:
            ...
            neutral_path = None
            if managed:
                neutral_path = self._neutral_attempt(nonce, params["ATTEMPT_ROOT"], neutral_state); self.sink_lock.acquire(); sink_locked = True
