...
class Supervisor:
    ...
    def _managed_object(self, path: str, directory: bool, identity: list[int], mode=None) -> None:
        ...
        try:
            ...
            for index, component in enumerate(names):
                ...
                child = os.open(component, dflags if want else os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0) | getattr(os, "O_CLOEXEC", 0), dir_fd=fd); opened = os.fstat(child)
                if (before.st_dev, before.st_ino, before.st_mode) != (opened.st_dev, opened.st_ino, opened.st_mode) or (want and not stat.S_ISDIR(opened.st_mode)): os.close(child); raise ValueError("managed ancestor changed")
    ...
    def _neutral_attempt(self, request_nonce: str, attempt: str, state: dict):
        flags = os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0) | getattr(os, "O_CLOEXEC", 0); attempt_fd = os.open(attempt, flags); state.update({"attempt_fd": attempt_fd, "origin_fd": -1, "directory_fd": -1, "directory_identity": None, "directory_created": False, "source_identities": {}, "pending_source_fd": -1}); origin_fd = os.open(os.path.join(self.cfg["root"], "k"), flags); state["origin_fd"] = origin_fd
        ...
        os.mkdir(name, 0o700, dir_fd=origin_fd); state["directory_created"] = True; created = os.stat(name, dir_fd=origin_fd, follow_symlinks=False); state["directory_identity"] = (created.st_dev, created.st_ino); directory_fd = os.open(name, flags, dir_fd=origin_fd); state["directory_fd"] = directory_fd; directory = os.fstat(directory_fd)
        ...
        for source in ("write-source", "rename-source", "unlink-source", "hardlink-source"):
            fd = os.open(source, os.O_WRONLY | os.O_CREAT | os.O_EXCL | os.O_NOFOLLOW | getattr(os, "O_CLOEXEC", 0), 0o600, dir_fd=directory_fd); state["pending_source_fd"] = fd; item = os.fstat(fd); state["source_identities"][source] = (item.st_dev, item.st_ino)
            ...
            os.write(fd, b"ORIGINAL"); os.fsync(fd); os.close(fd); state["pending_source_fd"] = -1
        ...
        for source, identity in state["source_identities"].items():
            before = os.stat(source, dir_fd=directory_fd, follow_symlinks=False); fd = os.open(source, os.O_RDONLY | os.O_NOFOLLOW | getattr(os, "O_CLOEXEC", 0), dir_fd=directory_fd); opened = os.fstat(fd); content = os.read(fd, 9); os.close(fd)
    ...
    def _cleanup_neutral(self, state) -> bool:
        ...
        ok, found, seen = not created or directory_identity is not None, not created, set()
        if pending_fd >= 0:
                ...
                for entry in os.listdir(parent_fd) if parent_fd >= 0 and directory_identity is not None else ():
                    try:
                        ...
                        if stat.S_ISDIR(item.st_mode) and (item.st_dev, item.st_ino) == directory_identity:
                            found = True; ok = ok and item.st_uid == os.geteuid() and stat.S_IMODE(item.st_mode) == 0o700
                            try: os.rmdir(entry, dir_fd=parent_fd)
                            except OSError as exc: ok = ok and exc.errno in (errno.ENOTEMPTY, errno.EEXIST)
                            break
                    except OSError: ok = False
        except OSError: ok = False
        finally:
    ...
    def _run_launch(self, conn, header, fds, ctx, envelope):
        ...
        try:
            ...
            if managed: os.close(self._open_managed_sink(os.O_RDONLY, current, current_identity))
            ...
            if not self._cleanup_neutral(neutral_state): raise OSError("managed proof cleanup failed")
            neutral_state = None
            if managed: os.close(self._open_managed_sink(os.O_RDONLY, current, current_identity)); os.close(sink_fd); sink_fd = -1; self.sink_lock.release(); sink_locked = False
            released = True
            ...
            if not restored or not cleaned: self.destroy()
            with self.lock:
