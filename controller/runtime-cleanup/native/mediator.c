...
static int recover_terminal_create(Store*t,const Request*r){
  ...
  size_t n=0;
  int found=terminal_receipt(t,&terminal,&raw,&n);
  free(raw);
  if(found<=0)return found;
  const char*fixed[]={"a","c","q","r","s"},*state_names[]={"lock"},*receipt_names[]={"last",".last.tmp"};
  if(!exact_names(t->ns,fixed,5,31U)||!exact_names(t->s,state_names,1,1U)||!exact_empty(t->a)||!exact_empty(t->c)||!exact_empty(t->q)||!exact_names(t->r,receipt_names,2,2U))return -1;
  Receipt last;
  int has_last=last_present(t,&last,NULL,NULL);
  if(has_last<0)return -1;
  if((has_last==0&&terminal.sequence!=1)||(has_last==1&&last.sequence+1!=terminal.sequence))return -1;
...
static int canonical_absolute(const char *p){
  ...
  return strstr(p,"//")==NULL&&strstr(p,"/./")==NULL&&strstr(p,"/../")==NULL&&strcmp(p+length-2,"/.")&&strcmp(p+length-3,"/..");
}
