...
static int validate_bootstrap_staging(int project,const struct stat*self){
  struct stat project_stat;if(fstat(project,&project_stat)||!S_ISDIR(project_stat.st_mode)||project_stat.st_uid||project_stat.st_gid||(project_stat.st_mode&07777)!=0700)return 0;const char*project_names[]={"controller","mediator"},*controller_names[]={"runtime-cleanup"},*runtime_names[]={"install"};if(!exact_names(project,project_names,2,1U))return 0;int controller=fixed_dir_at(project,"controller",0,0,0700),runtime=-1,self_fd=-1;int ok=controller>=0;if(ok){runtime=fixed_dir_at(controller,"runtime-cleanup",0,0,0700);ok=runtime>=0&&exact_names(controller,controller_names,1,1U)&&exact_names(runtime,runtime_names,1,1U);}struct stat named;if(ok){self_fd=stable_named_fd(runtime,"install",O_RDONLY,0,0,0500,4194304,NULL,NULL,&named);ok=self_fd>=0&&same_stat(self,&named);}if(self_fd>=0&&close(self_fd))ok=0;if(runtime>=0&&close(runtime))ok=0;if(controller>=0&&close(controller))ok=0;return ok;
}
static int exact_regular_bytes(int parent,const char*name,mode_t mode,const unsigned char*raw,size_t n,int flags,struct stat*identity,int*opened){
  unsigned char*observed=NULL;size_t observed_n=0;struct stat value;int fd=stable_named_fd(parent,name,flags,0,0,mode,4194304,&observed,&observed_n,&value);int ok=fd>=0&&observed_n==n&&!memcmp(observed,raw,n);free(observed);if(!ok){if(fd>=0)close(fd);return 0;}if(identity)*identity=value;if(opened)*opened=fd;else if(close(fd))return 0;return 1;
}
static int make_final_directory(int parent,const char*name){
  int existing=fixed_dir_at(parent,name,0,0,0555);if(existing>=0){int ok=close(existing)==0;return ok;}if(errno!=ENOENT)return 0;mode_t previous=umask(0);if(previous!=077){umask(previous);return 0;}int made=mkdirat(parent,name,0555);mode_t during=umask(077);if(made||during!=0)return 0;int fd=fixed_dir_at(parent,name,0,0,0555);int ok=fd>=0&&fsync(fd)==0&&flush_dir(parent);if(fd>=0&&close(fd))ok=0;return ok;
}
...
static int stage_embedded_mediator(int project,const unsigned char*raw,size_t n){
  struct stat named;if(fstatat(project,"mediator",&named,AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH)==0){mode_t mode=named.st_mode&07777;if(mode!=0400&&mode!=04555)return 0;unsigned char*observed=NULL;size_t observed_n=0;struct stat identity;int fd=stable_named_fd(project,"mediator",mode==0400?O_RDWR:O_RDONLY,0,0,mode,4194304,&observed,&observed_n,&identity);if(fd<0)return 0;int prefix=observed_n<=n&&!memcmp(observed,raw,observed_n);free(observed);if(!prefix){close(fd);return 0;}if(mode==04555){int ok=observed_n==n&&close(fd)==0;return ok;}if(observed_n<n){if(close(fd)||unlinkat(project,"mediator",AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH|AT_UNIQUE)||!flush_dir(project))return 0;}else{struct stat ready,rebound;int ok=!fsync(fd)&&!fchmod(fd,04555)&&!fsync(fd)&&!fstat(fd,&ready)&&S_ISREG(ready.st_mode)&&ready.st_uid==0&&ready.st_gid==0&&(ready.st_mode&07777)==04555&&ready.st_nlink==1&&ready.st_size==(off_t)n&&!fstatat(project,"mediator",&rebound,AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH)&&same_stat(&ready,&rebound)&&flush_dir(project);if(close(fd))ok=0;return ok;}}
  else if(errno!=ENOENT)return 0;
  int fd=openat(project,"mediator",O_RDWR|O_CREAT|O_EXCL|O_CLOEXEC|O_NOFOLLOW_ANY|O_UNIQUE,0400);if(fd<0)return 0;unsigned char*reloaded=NULL;size_t reloaded_n=0;struct stat durable,ready,rebound;int ok=!fchown(fd,0,0)&&!fchmod(fd,0400)&&write_all(fd,raw,n)&&!fsync(fd)&&stable_read_fd(fd,4194304,&reloaded,&reloaded_n,&durable)&&reloaded_n==n&&!memcmp(reloaded,raw,n)&&durable.st_uid==0&&durable.st_gid==0&&(durable.st_mode&07777)==0400&&durable.st_nlink==1&&!fstatat(project,"mediator",&rebound,AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH)&&same_stat(&durable,&rebound);free(reloaded);if(ok)ok=!fchmod(fd,04555)&&!fsync(fd)&&!fstat(fd,&ready)&&(ready.st_mode&07777)==04555&&ready.st_dev==durable.st_dev&&ready.st_ino==durable.st_ino&&ready.st_uid==0&&ready.st_gid==0&&ready.st_nlink==1&&ready.st_size==(off_t)n&&!fstatat(project,"mediator",&rebound,AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH)&&same_stat(&ready,&rebound)&&flush_dir(project);if(close(fd))ok=0;return ok;
}
static int installed_mediator_status(int helpers,const unsigned char*raw,size_t n){
  struct stat named;if(fstatat(helpers,"se.nortropic.runtime-cleanup-mediator",&named,AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH)){return errno==ENOENT?0:-1;}return exact_regular_bytes(helpers,"se.nortropic.runtime-cleanup-mediator",04555,raw,n,O_RDONLY,NULL,NULL)?1:-1;
}
static int publish_staged_mediator(int project,int helpers,const unsigned char*raw,size_t n){
  int installed=installed_mediator_status(helpers,raw,n);if(installed<0)return 0;if(installed==1){struct stat staged;if(fstatat(project,"mediator",&staged,AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH)==0||errno!=ENOENT)return 0;return 1;}struct stat staged,project_stat,helpers_stat;int fd=-1;if(!exact_regular_bytes(project,"mediator",04555,raw,n,O_RDONLY,&staged,&fd)||fstat(project,&project_stat)||fstat(helpers,&helpers_stat)||staged.st_dev!=project_stat.st_dev||staged.st_dev!=helpers_stat.st_dev){if(fd>=0)close(fd);return 0;}int flags=RENAME_EXCL|RENAME_NOFOLLOW_ANY|RENAME_RESOLVE_BENEATH;int ok=!renameatx_np(project,"mediator",helpers,"se.nortropic.runtime-cleanup-mediator",flags)&&flush_dir(project)&&flush_dir(helpers);struct stat retained,published;if(ok)ok=!fstat(fd,&retained)&&same_stat(&staged,&retained)&&!fstatat(helpers,"se.nortropic.runtime-cleanup-mediator",&published,AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH)&&same_stat(&retained,&published);if(close(fd))ok=0;return ok;
}
static int directory_prefix(int ns,int *prefix,int *has_lock){
  const char*dirs[]={"a","c","q","r","s"};int count=0,missing=0,lock=0;for(size_t i=0;i<5;i++){int fd=fixed_dir_at(ns,dirs[i],0,0,0555);if(fd<0){if(errno!=ENOENT)return 0;missing=1;continue;}if(missing){close(fd);return 0;}count++;if(i<4){if(!exact_empty(fd)){close(fd);return 0;}}else{const char*names[]={"lock"};if(!exact_names(fd,names,1,0)){close(fd);return 0;}struct stat value;if(fstatat(fd,"lock",&value,AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH)==0){int lock_fd=stable_named_fd(fd,"lock",O_RDWR,0,0,0600,0,NULL,NULL,&value);if(lock_fd<0||value.st_size!=0){if(lock_fd>=0)close(lock_fd);close(fd);return 0;}lock=close(lock_fd)==0;}else if(errno!=ENOENT){close(fd);return 0;}}if(close(fd))return 0;}if(count<5&&lock)return 0;*prefix=count;*has_lock=lock;return 1;
}
static int remove_fixed_staging(int project){
  unsigned leaf=AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH|AT_UNIQUE,dir_flags=AT_REMOVEDIR|AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH;const char*project_names[]={"controller"},*controller_names[]={"runtime-cleanup"},*runtime_names[]={"install"};if(!exact_names(project,project_names,1,1U))return 0;int controller=fixed_dir_at(project,"controller",0,0,0700),runtime=-1;if(controller<0)goto fail;runtime=fixed_dir_at(controller,"runtime-cleanup",0,0,0700);if(runtime<0||!exact_names(controller,controller_names,1,1U)||!exact_names(runtime,runtime_names,1,1U))goto fail;if(unlinkat(runtime,"install",leaf)||!flush_dir(runtime)||close(runtime))goto fail;runtime=-1;if(unlinkat(controller,"runtime-cleanup",dir_flags)||!flush_dir(controller)||close(controller))goto fail;controller=-1;if(unlinkat(project,"controller",dir_flags)||!flush_dir(project))goto fail;return exact_empty(project);
fail:if(runtime>=0)close(runtime);if(controller>=0)close(controller);return 0;
}
static int apply_install(int project,const unsigned char *mediator,size_t mediator_n){
  int ns=open(NS,O_RDONLY|O_DIRECTORY|O_CLOEXEC|O_NOFOLLOW_ANY);struct stat nss,named_ns,project_stat,staging_stat;if(ns<0||fstat(ns,&nss)||lstat(NS,&named_ns)||!same_stat(&nss,&named_ns)||!S_ISDIR(nss.st_mode)||nss.st_uid||nss.st_gid||((nss.st_mode&07777)!=0700&&(nss.st_mode&07777)!=0555)||fstat(project,&project_stat)||fstatat(ns,".install",&staging_stat,AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH)||!same_stat(&project_stat,&staging_stat)){if(ns>=0)close(ns);return 0;}int finalizing=(nss.st_mode&07777)==0555;const char*namespace_names[]={".install","a","c","q","r","s"};if(!exact_names(ns,namespace_names,6,1U)){close(ns);return 0;}int prefix=0,has_lock=0;if(!directory_prefix(ns,&prefix,&has_lock)){close(ns);return 0;}
...
static int installer_bootstrap(int argc,char **argv){
  if(argc!=2||!argv||!argv[0]||strcmp(argv[1],"bootstrap")||getuid()!=0||geteuid()!=0||getgid()!=0||getegid()!=0)return 1;char initial[4096];uint32_t cap=4096;if(_NSGetExecutablePath(initial,&cap)||!canonical_absolute(initial)||strcmp(initial,NS "/.install/controller/runtime-cleanup/install")||strcmp(argv[0],initial))return 1;static char*empty[]={NULL};environ=empty;if(chdir("/"))return 1;errno=0;if(fcntl(0,F_GETFD)!=-1||errno!=EBADF)return 1;closefrom(0);umask(077);char self_path[4096];struct stat self;int self_parent=-1,controller=-1,project=-1;unsigned char*mediator=NULL;size_t mediator_n=0;int result=1;if(!loaded_self(self_path,&self,&self_parent)||strcmp(self_path,initial)||self.st_uid!=0||self.st_gid!=0||(self.st_mode&07777)!=0500||self.st_nlink!=1)goto done;if(!load_embedded_mediator(&mediator,&mediator_n))goto done;controller=parent_dir(self_parent,0,0,0700);if(controller<0)goto done;project=parent_dir(controller,0,0,0700);if(project<0||!validate_bootstrap_staging(project,&self))goto done;result=!apply_install(project,mediator,mediator_n);
