...
static int parse_state_raw(const unsigned char*raw,size_t n,State*s){
  ...
  if(phase==4||phase==5){if(!s->socket_dev||!s->socket_ino)return 0;}
...
static int validate_store_shape(Store*t,const State*s){
  ...
  if(located==1){int object=open_exact_dir(parent,name,0,0,phase<=1?0700:0555,s->object_dev,s->object_ino);if(object<0)return 0;int object_ok=phase!=0||exact_empty(object);if(close(object))object_ok=0;if(!object_ok)return 0;}
  if(has_temp==1){if(phase!=7||strcmp(temp.runtime,s->runtime)||strcmp(temp.cap,s->cap)||strcmp(temp.create_nonce,s->create_nonce)||strcmp(temp.cleanup_nonce,s->cleanup_nonce)||temp.sequence!=s->sequence)return 0;}return 1;
}
...
static int valid_state_transition(const State*a,const State*b){
  ...
  if(old==6&&next==7)return !strcmp(a->cleanup_nonce,b->cleanup_nonce)&&state_ids_equal(a,b);
  if(old==7&&next==7)return lower_hex(a->cleanup_nonce,64)&&lower_hex(b->cleanup_nonce,64)&&strcmp(a->cleanup_nonce,b->cleanup_nonce)&&state_ids_equal(a,b);
  return 0;
}
...
static int reconcile_state_temp(Store*t){
  unsigned char*tmp=NULL,*cur=NULL;size_t tn=0,cn=0;struct stat id;int has_tmp=read_named(t->s,".current.tmp",&tmp,&tn,&id);if(has_tmp<0)return 0;if(has_tmp==0)return 1;State next,old;if(!parse_state_raw(tmp,tn,&next)){free(tmp);return 0;}free(tmp);int has_cur=read_named(t->s,"current",&cur,&cn,&id);if(has_cur<0)return 0;if(has_cur==0){if(strcmp(next.phase,"RESERVING"))return 0;return publish_temp(t->s,".current.tmp","current",1);}int ok=parse_state_raw(cur,cn,&old);free(cur);if(!ok||!valid_state_transition(&old,&next))return 0;return publish_temp(t->s,".current.tmp","current",0);
}
...
static int recover_terminal_create(Store*t,const Request*r){
  Receipt terminal;unsigned char*raw=NULL;size_t n=0;int found=terminal_receipt(t,&terminal,&raw,&n);free(raw);if(found<=0)return found;if(!strcmp(r->nonce,terminal.create_nonce)||!strcmp(r->nonce,terminal.cleanup_nonce)){error_frame(0,r->nonce,"REPLAY");return 2;}const char*fixed[]={"a","c","q","r","s"},*state_names[]={"lock"},*receipt_names[]={"last",".last.tmp"};if(!exact_names(t->ns,fixed,5,31U)||!exact_names(t->s,state_names,1,1U)||!exact_empty(t->a)||!exact_empty(t->c)||!exact_empty(t->q)||!exact_names(t->r,receipt_names,2,2U))return -1;Receipt last;int has_last=last_present(t,&last,NULL,NULL);if(has_last<0||(has_last==0&&terminal.sequence!=1)||(has_last==1&&last.sequence+1!=terminal.sequence))return -1;if(!publish_temp(t->r,".last.tmp","last",has_last==0))return -1;error_frame(0,r->nonce,"RECOVERED_RETRY");return 2;
}
static int process_create(Store*t,const Request*r){
  State current;int present=state_present(t,&current);if(present<0)return 1;if(!present){int terminal=recover_terminal_create(t,r);if(terminal<0)return 1;if(terminal==2)return 1;if(!validate_idle_store(t))return 1;}if(present&&!validate_store_shape(t,&current))return 1;int replay=nonce_replayed(t,present?&current:NULL,r->nonce);if(replay<0)return 1;if(replay)return error_frame(0,r->nonce,"REPLAY")?1:1;
...
static int process_cleanup(Store*t,const Request*r){
  State current;int present=state_present(t,&current);if(present<0)return 1;if(present&&!validate_store_shape(t,&current))return 1;int replay=nonce_replayed(t,present?&current:NULL,r->nonce);if(replay<0)return 1;if(replay){error_frame(0,r->nonce,"REPLAY");return 1;}if(!present||!current_capability(&current,r->capability)){error_frame(0,r->nonce,"INVALID_CAPABILITY");return 1;}if(strcmp(current.phase,"ACTIVE")&&strcmp(current.phase,"QUARANTINING")&&strcmp(current.phase,"CLEANING")){error_frame(0,r->nonce,"INVALID_CAPABILITY");return 1;}
...
static int validate_staging(int project,int applying,const struct stat*self,const struct stat*source){
  uid_t uid=applying?0:getuid();gid_t gid=applying?0:(gid_t)-1;mode_t directory_mode=applying?0700:0755;struct stat project_stat;if(fstat(project,&project_stat)||!S_ISDIR(project_stat.st_mode)||project_stat.st_uid!=uid||(applying&&project_stat.st_gid!=0)||(project_stat.st_mode&07777)!=0700)return 0;
  ...
  int controller=fixed_dir_at(project,"controller",uid,gid,directory_mode),verify=fixed_dir_at(project,"verify",uid,gid,directory_mode),runtime=-1,h039=-1,self_fd=-1,source_fd=-1;int ok=controller>=0&&verify>=0;if(ok){runtime=fixed_dir_at(controller,"runtime-cleanup",uid,gid,directory_mode);h039=fixed_dir_at(verify,"h039",uid,gid,directory_mode);ok=runtime>=0&&h039>=0;}struct stat self_named,source_named;if(ok)ok=exact_names(controller,controller_names,1,1U)&&exact_names(runtime,runtime_names,1,1U)&&exact_names(verify,verify_names,1,1U)&&exact_names(h039,h039_names,1,1U);if(ok){self_fd=stable_named_fd(runtime,"install",O_RDONLY,uid,gid,applying?0500:0755,4194304,NULL,NULL,&self_named);source_fd=stable_named_fd(h039,"runtime-cleanup-mediator",O_RDONLY,uid,gid,applying?0400:0555,4194304,NULL,NULL,&source_named);ok=self_fd>=0&&source_fd>=0&&same_stat(self,&self_named)&&same_stat(source,&source_named);}if(self_fd>=0&&close(self_fd))ok=0;if(source_fd>=0&&close(source_fd))ok=0;if(runtime>=0&&close(runtime))ok=0;if(h039>=0&&close(h039))ok=0;if(controller>=0&&close(controller))ok=0;if(verify>=0&&close(verify))ok=0;return ok;
}
static int validate_recovery_staging(int project,const struct stat*self){
  struct stat project_stat;if(fstat(project,&project_stat)||!S_ISDIR(project_stat.st_mode)||project_stat.st_uid||project_stat.st_gid||(project_stat.st_mode&07777)!=0700)return 0;const char*project_names[]={"controller","verify"},*controller_names[]={"runtime-cleanup"},*runtime_names[]={"install"},*verify_names[]={"h039"};if(!exact_names(project,project_names,2,1U))return 0;
  int controller=fixed_dir_at(project,"controller",0,0,0700),runtime=-1,verify=-1,h039=-1,self_fd=-1;int ok=controller>=0;if(ok){runtime=fixed_dir_at(controller,"runtime-cleanup",0,0,0700);ok=runtime>=0&&exact_names(controller,controller_names,1,1U)&&exact_names(runtime,runtime_names,1,1U);}struct stat named;if(ok){self_fd=stable_named_fd(runtime,"install",O_RDONLY,0,0,0500,4194304,NULL,NULL,&named);ok=self_fd>=0&&same_stat(self,&named);}if(ok){verify=fixed_dir_at(project,"verify",0,0,0700);if(verify<0&&errno!=ENOENT)ok=0;else if(verify>=0){h039=fixed_dir_at(verify,"h039",0,0,0700);if(h039<0&&errno!=ENOENT)ok=0;else if(h039>=0)ok=exact_empty(h039)&&exact_names(verify,verify_names,1,1U);else ok=exact_empty(verify);}}
  if(self_fd>=0&&close(self_fd))ok=0;if(h039>=0&&close(h039))ok=0;if(verify>=0&&close(verify))ok=0;if(runtime>=0&&close(runtime))ok=0;if(controller>=0&&close(controller))ok=0;return ok;
}
static int load_installed_source(unsigned char**raw,size_t*length){int helpers=open("/Library/PrivilegedHelperTools",O_RDONLY|O_DIRECTORY|O_CLOEXEC|O_NOFOLLOW_ANY);if(helpers<0)return 0;struct stat identity;int fd=stable_named_fd(helpers,"se.nortropic.runtime-cleanup-mediator",O_RDONLY,0,0,04555,4194304,raw,length,&identity);int ok=fd>=0&&*length>0;if(fd>=0&&close(fd))ok=0;if(close(helpers))ok=0;char sum[65];if(ok)digest(*raw,*length,sum);if(!ok||strcmp(sum,Q(H039_MEDIATOR_SHA256))){free(*raw);*raw=NULL;return 0;}return 1;}
static int ensure_dir(int parent,const char*name,mode_t mode){int fd=fixed_dir_at(parent,name,0,0,mode);if(fd>=0){int ok=close(fd)==0;return ok;}if(errno!=ENOENT)return 0;return make_dir(parent,name,mode);}
static int ensure_lock(int state){struct stat identity;int fd=stable_named_fd(state,"lock",O_RDWR,0,0,0600,0,NULL,NULL,&identity);if(fd>=0){int ok=identity.st_size==0&&close(fd)==0;return ok;}if(errno!=ENOENT)return 0;fd=openat(state,"lock",O_RDWR|O_CREAT|O_EXCL|O_CLOEXEC|O_NOFOLLOW_ANY|O_UNIQUE,0600);struct stat rebound;int ok=fd>=0&&!fchown(fd,0,0)&&!fchmod(fd,0600)&&!fsync(fd)&&!fstat(fd,&identity)&&identity.st_size==0&&S_ISREG(identity.st_mode)&&identity.st_uid==0&&identity.st_gid==0&&(identity.st_mode&07777)==0600&&identity.st_nlink==1&&!fstatat(state,"lock",&rebound,AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH)&&same_stat(&identity,&rebound);if(fd>=0&&close(fd))ok=0;return ok&&flush_dir(state);}
static int ensure_installed(int helpers,const unsigned char*raw,size_t n){unsigned char*existing=NULL;size_t existing_n=0;struct stat identity;int fd=stable_named_fd(helpers,"se.nortropic.runtime-cleanup-mediator",O_RDONLY,0,0,04555,4194304,&existing,&existing_n,&identity);if(fd>=0){int ok=existing_n==n&&!memcmp(existing,raw,n)&&close(fd)==0;free(existing);return ok;}if(errno!=ENOENT)return 0;return copy_exclusive(helpers,"se.nortropic.runtime-cleanup-mediator",raw,n,04555)&&flush_dir(helpers);}
static int remove_fixed_staging(int project){
  unsigned leaf=AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH|AT_UNIQUE,dir_flags=AT_REMOVEDIR|AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH;const char*project_names[]={"controller","verify"},*controller_names[]={"runtime-cleanup"},*runtime_names[]={"install"},*verify_names[]={"h039"},*source_names[]={"runtime-cleanup-mediator"};if(!exact_names(project,project_names,2,1U))return 0;int controller=fixed_dir_at(project,"controller",0,0,0700),runtime=-1,verify=-1,h039=-1;if(controller<0)goto fail;runtime=fixed_dir_at(controller,"runtime-cleanup",0,0,0700);if(runtime<0||!exact_names(controller,controller_names,1,1U)||!exact_names(runtime,runtime_names,1,1U))goto fail;
  verify=fixed_dir_at(project,"verify",0,0,0700);if(verify<0&&errno!=ENOENT)goto fail;if(verify>=0){if(!exact_names(verify,verify_names,1,0))goto fail;h039=fixed_dir_at(verify,"h039",0,0,0700);if(h039<0&&errno!=ENOENT)goto fail;if(h039>=0){if(!exact_names(h039,source_names,1,0))goto fail;struct stat source;if(fstatat(h039,"runtime-cleanup-mediator",&source,AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH)==0){if(!S_ISREG(source.st_mode)||source.st_uid||source.st_gid||(source.st_mode&07777)!=0400||source.st_nlink!=1||unlinkat(h039,"runtime-cleanup-mediator",leaf)||!flush_dir(h039))goto fail;}else if(errno!=ENOENT)goto fail;if(!exact_empty(h039)||close(h039))goto fail;h039=-1;if(unlinkat(verify,"h039",dir_flags)||!flush_dir(verify))goto fail;}if(!exact_empty(verify)||close(verify))goto fail;verify=-1;if(unlinkat(project,"verify",dir_flags)||!flush_dir(project))goto fail;}
  if(unlinkat(runtime,"install",leaf)||!flush_dir(runtime)||close(runtime))goto fail;runtime=-1;if(unlinkat(controller,"runtime-cleanup",dir_flags)||!flush_dir(controller)||close(controller))goto fail;controller=-1;if(unlinkat(project,"controller",dir_flags)||!flush_dir(project))goto fail;return exact_empty(project);
fail:if(h039>=0)close(h039);if(runtime>=0)close(runtime);if(verify>=0)close(verify);if(controller>=0)close(controller);return 0;
}
static int apply_install(int project,const unsigned char *mediator,size_t mediator_n){
  int ns=open(NS,O_RDONLY|O_DIRECTORY|O_CLOEXEC|O_NOFOLLOW_ANY);struct stat nss,project_stat,staging_stat;if(ns<0||fstat(ns,&nss)||!S_ISDIR(nss.st_mode)||nss.st_uid||nss.st_gid||((nss.st_mode&07777)!=0700&&(nss.st_mode&07777)!=0555)||fstat(project,&project_stat)||fstatat(ns,".install",&staging_stat,AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH)||!same_stat(&project_stat,&staging_stat)){if(ns>=0)close(ns);return 0;}int finalizing=(nss.st_mode&07777)==0555;const char*namespace_names[]={".install","a","c","q","r","s"};if(!exact_names(ns,namespace_names,6,finalizing?63U:1U)){close(ns);return 0;}
  const char *dirs[]={"a","c","q","r","s"};if(!finalizing)for(size_t i=0;i<5;i++)if(!ensure_dir(ns,dirs[i],0555)){close(ns);return 0;}int a=fixed_dir_at(ns,"a",0,0,0555),c=fixed_dir_at(ns,"c",0,0,0555),q=fixed_dir_at(ns,"q",0,0,0555),r=fixed_dir_at(ns,"r",0,0,0555),s=fixed_dir_at(ns,"s",0,0,0555);int ok=a>=0&&c>=0&&q>=0&&r>=0&&s>=0&&exact_empty(a)&&exact_empty(c)&&exact_empty(q)&&exact_empty(r)&&ensure_lock(s);const char*lock_name[]={"lock"};if(ok)ok=exact_names(s,lock_name,1,1U)&&exact_names(ns,namespace_names,6,63U);if(a>=0)close(a);if(c>=0)close(c);if(q>=0)close(q);if(r>=0)close(r);if(s>=0)close(s);if(!ok){close(ns);return 0;}
  int helpers=open("/Library/PrivilegedHelperTools",O_RDONLY|O_DIRECTORY|O_CLOEXEC|O_NOFOLLOW_ANY);struct stat hs;if(helpers<0||fstat(helpers,&hs)||!S_ISDIR(hs.st_mode)||hs.st_uid||hs.st_gid||(hs.st_mode&0022)){if(helpers>=0)close(helpers);close(ns);return 0;}ok=ensure_installed(helpers,mediator,mediator_n);if(close(helpers))ok=0;if(!ok){close(ns);return 0;}
  struct stat published_fd,published_name;if(!finalizing&&(fchmod(ns,0555)||fsync(ns))) {close(ns);return 0;}if(fstat(ns,&published_fd)||lstat(NS,&published_name)||!same_stat(&published_fd,&published_name)||!S_ISDIR(published_fd.st_mode)||published_fd.st_uid||published_fd.st_gid||(published_fd.st_mode&07777)!=0555){close(ns);return 0;}
  if(!remove_fixed_staging(project)||unlinkat(ns,".install",AT_REMOVEDIR|AT_SYMLINK_NOFOLLOW_ANY|AT_RESOLVE_BENEATH)||!flush_dir(ns)||!exact_names(ns,dirs,5,31U)||fstat(ns,&published_fd)||lstat(NS,&published_name)||!same_stat(&published_fd,&published_name)||(published_fd.st_mode&07777)!=0555){close(ns);return 0;}close(ns);return 1;
}
static int installer_main(int argc,char **argv){
  if(argc!=2||(!argv)||(!argv[0])||(strcmp(argv[1],"verify")&&strcmp(argv[1],"apply")))return 1;int applying=!strcmp(argv[1],"apply");char self_path[4096];struct stat self,source;int self_parent=-1,controller=-1,project=-1;unsigned char*raw=NULL;size_t raw_n=0;int result=1;
  ...
  int source_status=load_source(project,uid,gid,source_mode,self.st_dev,&raw,&raw_n,&source);if(source_status==1){if(!validate_staging(project,applying,&self,&source))goto done;}else if(source_status==0&&applying){if(!validate_recovery_staging(project,&self)||!load_installed_source(&raw,&raw_n))goto done;}else goto done;
  result=applying?!apply_install(project,raw,raw_n):0;
done:if(project>=0)close(project);if(controller>=0)close(controller);if(self_parent>=0)close(self_parent);free(raw);return result;
}
#endif
