...
static int process_cleanup(Store*t,const Request*r){
  State current;int present=state_present(t,&current);if(present<0)return 1;if(!present){int handled=0,rc=recover_terminal_cleanup(t,r,&handled);if(handled)return rc;if(!validate_idle_store(t))return 1;int replay=nonce_replayed(t,NULL,r->nonce);if(replay<0)return 1;if(replay){error_frame(0,r->nonce,"REPLAY");return 1;}error_frame(0,r->nonce,"INVALID_CAPABILITY");return 1;}int receipt_temp_present=0;if(!validate_store_shape(t,&current,&receipt_temp_present))return 1;int temp_continuation=receipt_temp_present&&!strcmp(current.cleanup_nonce,r->nonce);if(!temp_continuation){int replay=nonce_replayed(t,&current,r->nonce);if(replay<0)return 1;if(replay){error_frame(0,r->nonce,"REPLAY");return 1;}}if(!current_capability(&current,r->capability)){error_frame(0,r->nonce,"INVALID_CAPABILITY");return 1;}if(receipt_temp_present&&!temp_continuation){error_frame(0,r->nonce,"CAPACITY");return 1;}if(strcmp(current.phase,"ACTIVE")&&strcmp(current.phase,"QUARANTINING")&&strcmp(current.phase,"CLEANING")){error_frame(0,r->nonce,"INVALID_CAPABILITY");return 1;}
  const char*reason="INCOMPLETE_IDENTITY";int reader=-1,eof=fifo_eof_for(t,&current,&reader,&reason);if(eof==0){error_frame(0,r->nonce,"LEASE_LIVE");return 1;}if(eof<0){error_frame(0,r->nonce,reason);return 1;}if(!receipt_temp_present){strcpy(current.cleanup_nonce,r->nonce);if(!strcmp(current.phase,"CLEANING")){if(!put_state(t,&current,0)){if(reader>=0)close(reader);error_frame(0,r->nonce,"INCOMPLETE_IDENTITY");return 1;}}else if(!quarantine(t,&current,&reason)){if(reader>=0)close(reader);error_frame(0,r->nonce,reason);return 1;}}
