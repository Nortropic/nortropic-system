...
if ! g rev-parse -q --verify "refs/remotes/origin/$GREN" >/dev/null; then
  [ "$TORR" = 1 ] && echo "torr: skulle pusha $GREN" || { g push -q -u origin "$GREN" || stopp "push misslyckades — commiten är LOKAL, inget publicerat"; }
fi
FJARR="$(g rev-parse -q --verify "refs/remotes/origin/$GREN" 2>/dev/null || echo '')"
if [ "$FJARR" != "$HEAD_SHA" ]; then
...
case "$GRANSKARE" in
  ...
  ci)
    # Kvittot kommer från en befintlig PR-review-kommentar (t.ex. CI-jobbet, när det är påslaget).
    DOM="$($GH api "repos/$REPO/pulls/$PRNR/reviews" --jq '.[].body' 2>/dev/null | grep -E '^DOM: (TILLSTYRKS|FYND) @[0-9a-f]{7,40}' | tail -1)"
    [ -n "$DOM" ] || odombart "ingen DOM-rad i PR #$PRNR:s reviews"
    ;;
  claude)
    ...
    DOM="$(grep -E '^DOM: (TILLSTYRKS|FYND) @[0-9a-f]{7,40}' "$RAPPORT" | tail -1)"
    [ -n "$DOM" ] || odombart "ingen DOM-rad i granskarens svar ($RAPPORT)"
    $GH pr review "$PRNR" --repo "$REPO" --comment --body-file "$RAPPORT" >/dev/null 2>&1 || echo "  varning: kunde inte lägga rapporten som PR-review — den finns lokalt i $RAPPORT" >&2
    ;;
  *) echo "okänd granskare: $GRANSKARE" >&2; exit 3 ;;
esac
DOMSHA="$(printf '%s\n' "$DOM" | sed -E 's/^DOM: [A-Z]+ @([0-9a-f]+).*/\1/')"
case "$HEAD_SHA" in "$DOMSHA"*) ;; *) stopp "domen gäller $DOMSHA, inte HEAD $HEAD_SHA — gammal granskning gäller inte ny kandidat" ;; esac
...
CHECKS="$($GH pr view "$PRNR" --repo "$REPO" --json statusCheckRollup --jq '.statusCheckRollup[] | "\(.name // .context // "?")\t\(.conclusion // .state // "?")"' 2>/dev/null || echo '')"
if [ -n "$CHECKS" ]; then
  RODA="$(printf '%s\n' "$CHECKS" | awk -F'\t' 'tolower($2)!="success" && tolower($2)!="skipped" && tolower($2)!="neutral" {print $1" = "$2}')"
  if [ -n "$RODA" ]; then
    if [ -n "$ODOK" ]; then
      NAMN="${ODOK%%:*}"
      KVAR="$(printf '%s\n' "$RODA" | grep -v -F "$NAMN" || true)"
