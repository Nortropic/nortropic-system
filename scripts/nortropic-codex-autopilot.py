...
def _retire_bound_staging(staging_dir_fd, root, identity):
    ...
    try:
        ...
        pass
    # R116: retire the relocated staging directory itself by the bound
    ...
    # bound inode's current path, re-verify identity, and remove that tree.
    try:
        moved = os.fsdecode(
            fcntl.fcntl(staging_dir_fd, getattr(fcntl, "F_GETPATH", 50),
                        b"\0" * 1024).split(b"\0", 1)[0])
        moved_now = os.lstat(moved)
        if ((moved_now.st_dev, moved_now.st_ino) == identity
                and stat.S_ISDIR(moved_now.st_mode) and not os.path.islink(moved)):
            shutil.rmtree(moved)
    except OSError:
        pass
    return False
