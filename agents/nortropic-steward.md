...
<ROT>/vendored-skills/   facit-kopior av de 9 load-bearing tredjepartsskillsen
...
9. **Härledd-kopia-integritet (R1 — repo-nativ semantik):** repot ÄR auktoriteten; varje INSTALLERAD/HÄRLEDD kopia av en load-bearing-skill (t.ex. en harness-install under `~/.claude/skills/<n>` eller annan runtime-plats) ska vara byte-identisk med repots yta. För var och en av de 9 load-bearing-skillsen (se #3): finns en härledd kopia utanför repot → `diff -r --exclude=__pycache__ --exclude='*.pyc' --exclude=VENDORED.md <härledd-kopia> $ROT/vendored-skills/<n>` — diff ≠ tom → **WARN** ("härledd kopia driftad — repot vinner: ompinna kopian, eller granska om drift kom från en auto-uppdatering"). Finns INGEN härledd kopia (repo-nativ drift, agenterna laddar direkt ur repot) → **PASS med notering "repo-nativ: ingen härledd kopia"** — det är det förväntade normaltillståndet efter S0. Saknas repots `$ROT/vendored-skills/<n>`-yta → **FAIL**. *(Mekanisk hash-verifiering av vendored-ytorna själva = R5.)*
